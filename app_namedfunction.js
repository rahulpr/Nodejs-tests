...
var renderPage = function (res, page) {
    ...
    // functions below are local to renderPage
    var readfileCbk = function (err, pageData) {
        if (err) {
            console.log(err);
    ...
    var fileexistsCbk = function (exists) {
        if (exists) {
            fs.readFile(filePath, readfileCbk);
        } else {
            res.end('Page does not exist.');
        }
    };
    
    fs.exists(filePath, fileexistsCbk);
};
